...
public class GoldCrop extends CropBase implements IHaveRecipe {
	public GoldCrop() {
		super("GoldCrop", Items.GOLD_INGOT, Blocks.NETHERRACK);
	}
	...
	public List<ItemStack> getDrops(IBlockAccess world, BlockPos pos, IBlockState state, int fortune) {
		...
		if (age >= 7) {
			...
			if (rand.nextInt(100) + 1 <= ModMain.config.getInt(Constants.GOLDSEEDCHANCE)) {
				ret.add(new ItemStack(this.getSeed(), 1));
				if (ModMain.config.getBoolean(Constants.SEEDDROP))
					ret.add(new ItemStack(Items.GOLD_NUGGET, 2));
			}
		}
		return ret;
	}
	...
	public void addRecipeToList(NonNullList<IRecipe> recipeList) {
		IRecipe recipe = new RecipeBuilder(this.getSeed()).map("ggg", "gng", "ggg").where('g', Blocks.GOLD_BLOCK)
