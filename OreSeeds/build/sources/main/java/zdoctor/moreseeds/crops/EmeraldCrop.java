...
public class EmeraldCrop extends CropBase implements IHaveRecipe {
	public EmeraldCrop() {
		super("EmeraldCrop", Items.EMERALD, Blocks.NETHERRACK);
	}
	...
	public List<ItemStack> getDrops(IBlockAccess world, BlockPos pos, IBlockState state, int fortune) {
		...
		if (age >= 7) {
			...
			if (rand.nextInt(100) + 1 <= ModMain.config.getInt(Constants.EMERALDSEEDCHANCE)) {
	...
	public void addRecipeToList(NonNullList<IRecipe> recipeList) {
		IRecipe recipe = new RecipeBuilder(this.getSeed()).map("eee", "ene", "eee").where('e', Blocks.EMERALD_BLOCK)
